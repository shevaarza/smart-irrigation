...
import { WateringSchedule } from '@/lib/types';
...
export default function ScheduleForm({ schedules, onRefresh }: Props) {
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch('/api/schedule', {
        ...
        body: JSON.stringify({ hour: +hour, minute: +minute, duration_sec: +duration, enabled }),
      });
  ...
  const handleToggle = async (s: WateringSchedule) => {
    const res = await fetch('/api/schedule', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: s.id, hour: s.hour, minute: s.minute, duration_sec: s.duration_sec, enabled: !s.enabled }),
    });
    const json = await res.json();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-yellow-50 border border-yellow-100 rounded-2xl p-5">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <div className="flex flex-col justify-end">
            ...
            <button type="button" onClick={() => setEnabled(!enabled)}
              className={`px-3 py-2 rounded-xl text-xs font-semibold border transition ${enabled ? 'bg-green-100 text-green-700 border-green-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
        ...
        {msg && (
          <div className={`mt-3 px-3 py-2 rounded-xl text-xs font-medium ${msg.type === 'ok' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            {msg.text}
          </div>
        )}
      </div>
      ...
      <div className="space-y-2">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Jadwal Tersimpan di Database</p>
        {schedules.length === 0 && (
          <p className="text-sm text-slate-400 py-3 text-center">Belum ada jadwal</p>
        )}
        {schedules.map(s => (
          <div key={s.id} className={`flex items-center justify-between px-4 py-3 rounded-xl border ${s.enabled ? 'bg-white border-green-100' : 'bg-slate-50 border-slate-100'}`}>
            <div className="flex items-center gap-3">
              <div className={`w-2 h-2 rounded-full ${s.enabled ? 'bg-green-400' : 'bg-slate-300'}`}/>
              <div>
                <p className="text-sm font-bold text-slate-700">
                  {padZero(s.hour)}:{padZero(s.minute)}
                </p>
                <p className="text-xs text-slate-400">{s.duration_sec}s · {s.enabled ? 'Aktif' : 'Nonaktif'}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => handleToggle(s)}
                className={`text-xs px-2.5 py-1 rounded-lg font-medium transition ${s.enabled ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>
